                        ...
                        std::ofstream& csv)
{
    ...
    try {
        rtwbs::TimedAutomaton ta_pt(t.pt_path);
        rtwbs::TimedAutomaton ta_dt(t.dt_path);
        ...
        if (syntactic) {
            rtwbs::SemanticAlignmentChecker base;
            rtwbs::SemanticAlignmentResult base_result;
            bool ok = base.check_weak_timed_bisimulation(ta_pt, ta_dt, base_result);
            std::cout << "  Syntactic RTWBS: " << (ok ? "TRUE" : "FALSE") << std::endl;
        ...
        rtwbs::DomainKnowledge dk = t.ont_path.empty()
                                   ? rtwbs::OntologyParser::make_trivial()
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "  Mode: " << (syntactic ? "syntactic RTWBS" : "semantic alignment")
              << std::endl;
