...
void write_csv_header(std::ofstream& file) {
    file << "system_1,system_2,refined_states,abstract_states,simulation_pairs,check_time_ms,memory_usage_bytes,memory_usage_kb,equivalent" << std::endl;
}
...
void append_to_csv(std::ofstream& file, 
    ...
    file << sys1 << "," << sys2 << ","
         ...
         << (are_equivalent ? ",EQUIVALENT" : ",NOT EQUIVALENT")
         << std::endl;
}
    ...
    const char* benchmark_prefix  )
{
        ...
        for (size_t i = 0; i < filenames.size(); ++i) {
            ...
            for (size_t j = i+1; j < filenames.size(); ++j) {  // Start from i+1
                ...
                bool equivalent = checker.check_rtwbs_equivalence(*systems_map[filename_1], *systems_map[filename_2]);
                std::cout << "Equivalence result: " << (equivalent ? "EQUIVALENT" : "NOT EQUIVALENT") << std::endl;
                checker.print_statistics();
